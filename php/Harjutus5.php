...
<html lang="en">
  ...
  <div class="container">
    ...
    <h2>Tüdrukud</h2>
  <?php 
   #Harjutus 5, Sten Veski, 26.01.2023

 
   $nimed = array('mari', 'kati', 'sandra', 'liisa', 'kertu', 'jana', 'pille', 'krista');
   sort($nimed);

   foreach($nimed as $nimi)
        echo $nimi . "<br>";
  echo $nimed [0] . ",". $nimed[1] . ",". $nimed[2] . "<br>";
  $randnimed = $nimed[array_rand($nimed)];
  echo $randnimed . "<br>"
...
<h2>Autod</h2>
<?php
$auto1 = array("Subaru","BMW","Acura","Mercedes-Benz","Lexus","GMC","Volvo","Toyota","Volkswagen","Volkswagen","GMC","Jeep","Saab","Hyundai","Subaru","Mercedes-Benz",
...
$auto2 = array("1GKS1GKC8FR966658", "1FTEW1C87AK375821", "1G4GF5E30DF760067", "1FTEW1CW9AF114701", "WAUGGAFC8CN433989", "3G5DA03E83S704506", "4JGDA2EB0DA207570", 
...
echo "Autode arv: " . count($auto1);
echo "<br> Toyotade arv: " . count(array_keys($auto1,"Toyota"));
echo "<br> Audide arv: " . count(array_keys($auto1,"Audi")). "<br>";

foreach($auto2 as $lol){
  if(strlen ($lol) <= 16){
    echo $lol. "<br>";
  }
}
...
$kpalk = array_sum($palk)/count($palk);
echo "Keskmine palk: " .$kpalk . "<br>";
...
sort($firmad);
if (isset($_POST["eemalda1"])){
  $firmad1 = $_POST["firma_nimi"];
  if (($key = array_search($firmad1, $firmad)) !== false){
    unset($firmad[$key]);
  }
}
foreach ($firmad as $firmad2){
  echo $firmad2 . "<br>";
}
echo "<br>";

?>
<form action="harjutus5.php" method="post">
    <input type="text" name="firma_nimi">
    <input type="submit" name="eemalda1" value="Eemalda">
    </form>
...
foreach($riigid as $riik1){
  if(strlen($riik1) > strlen($pikk)){
    $pikk = $riik1;
  }
}
echo $pikk;
...
<h2>Google</h2>
<form action="harjutus5.php" method="get">
    Sisesta nimi <input type="text" name="t2">
...
function findname($nimed2, $google) {
  if (in_array($nimed2, $google)) {
...
echo '<img class="col-sm-2" src="img/'.$pilt[2]. '">';
echo '<div class="row">';
